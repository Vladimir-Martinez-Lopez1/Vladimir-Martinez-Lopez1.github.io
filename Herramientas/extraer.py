    ...
    with path.open("r", encoding="utf-8", errors="ignore") as lineas:
        for linea in lineas: # Leer línea por línea
            ...
            if linea.startswith("v "):  # Vértice
                ...
                indices = [int(chunk.split('/')[0]) for chunk in linea.split()[1:]]
                faces.append(indices)
            elif linea.startswith("g "):  # Grupo/nombre de colección
                grupoActual = linea[2:].strip() or f"Grupo_{len(grupoNombres)+1}"
            elif linea.startswith("o "):  # Nombre de objeto
                grupoActual = linea[2:].strip() or f"Objeto_{len(grupoNombres)+1}"
...
def obtenerRutaUnica(obj_path: Path, output_dir: Path) -> Path:
    """Genera una ruta de salida única."""
    nombreObjeto = obj_path.stem
    contador = 0
    while True:
        output_name = f"{nombreObjeto}_estructurado.txt" if contador == 0 else f"{nombreObjeto}_estructurado_{contador}.txt"
...
def main():
    ...
    output_path = obtenerRutaUnica(obj_path, output_dir)
